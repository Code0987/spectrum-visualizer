...
class CloudVisualizer extends BaseVisualizer {
    ...
    draw(deltaTime) {
        ...
        // Re-init flow field if needed
        if (this.flowField.length === 0) {
            this.initFlowField();
        }
    ...
    updateFlowField(bands, deltaTime) {
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                field.angle +=
                    noise * deltaTime * speed +
                    bassInfluence * Math.sin(this.time + x * 0.1);
                field.strength =
                    0.3 +
                    midInfluence +
                    Math.abs(Math.sin(this.time * 0.5 + y * 0.1)) * 0.3;
            }
        }
    }
    ...
    drawBackground(colors, intensity) {
        ...
        const alpha = Math.floor((0.1 + intensity * 0.1) * 255)
            .toString(16)
            .padStart(2, "0");
        gradient.addColorStop(0, `${colors.primary}${alpha}`);
        gradient.addColorStop(
            0.5,
            `${colors.secondary}${Math.floor(parseInt(alpha, 16) * 0.5)
                .toString(16)
                .padStart(2, "0")}`,
        );
    ...
    spawnClouds(bands, colors) {
        ...
        if (
            ...
            const colorIndex = Math.floor(
                Math.random() * colors.gradient.length,
            );
    ...
    updateClouds(deltaTime) {
        ...
        for (let i = this.clouds.length - 1; i >= 0; i--) {
            ...
            cloud.x +=
                Math.cos(field.angle) * field.strength * speed * 50 * deltaTime;
            cloud.y +=
                Math.sin(field.angle) * field.strength * speed * 50 * deltaTime;
        ...
        for (const cloud of this.clouds) {
            ...
            for (const blob of cloud.blobs) {
                ...
                const blobSize =
                    blob.size *
                    cloud.size *
                    (0.8 + Math.sin(this.time + blob.phase) * 0.2);
                ...
                const colorAlpha = Math.floor(alpha * 0.6 * 255)
                    .toString(16)
                    .padStart(2, "0");
                gradient.addColorStop(0, `${cloud.color}${colorAlpha}`);
                gradient.addColorStop(
                    0.5,
                    `${cloud.color}${Math.floor(parseInt(colorAlpha, 16) * 0.5)
                        .toString(16)
                        .padStart(2, "0")}`,
                );
