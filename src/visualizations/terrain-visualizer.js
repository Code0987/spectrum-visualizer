...
class TerrainVisualizer extends BaseVisualizer {
  ...
  updateTerrain(data) {
    ...
    for (let x = 0; x < this.terrainWidth; x++) {
      ...
      const centerFactor = 1 - Math.abs(x - this.terrainWidth / 2) / (this.terrainWidth / 2);
      const height = value * 150 * centerFactor * this.settings.sensitivity;
  ...
  drawTerrain(colors, intensity) {
    ...
    for (let z = this.terrainDepth - 1; z >= 0; z--) {
      for (let x = 0; x < this.terrainWidth - 1; x++) {
        ...
        const h01 = z < this.terrainDepth - 1 ? this.terrain[z + 1][x] : 0;
        const h11 = z < this.terrainDepth - 1 ? this.terrain[z + 1][x + 1] : 0;
        ...
        if (heightFactor > 0.1) {
          ...
          gradient.addColorStop(0, `hsla(${280 + hue}, 100%, ${40 + heightFactor * 30}%, ${fillAlpha})`);
          gradient.addColorStop(1, `hsla(${200 + hue}, 100%, ${20 + heightFactor * 20}%, ${fillAlpha * 0.5})`);
          this.ctx.fillStyle = gradient;
          this.ctx.fill();
        }
  ...
  project(x, y, z) {
    ...
    const screenY = this.height * 0.7 - (py + this.cameraHeight) * scale + pz * 0.5;
  ...
  drawSun(colors, bands) {
    ...
    // Sun rings
    for (let i = 0; i < 3; i++) {
      const ringRadius = pulseRadius + 20 + i * 15 + Math.sin(this.time * 2 + i) * 5;
      const alpha = 0.3 - i * 0.08;

      this.ctx.beginPath();
      this.ctx.arc(sunX, sunY, ringRadius, 0, Math.PI * 2);
      this.ctx.strokeStyle = `${colors.secondary}${Math.floor(alpha * 255).toString(16).padStart(2, '0')}`;
  ...
  drawTerrainParticles(colors, intensity) {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const baseY = this.height * 0.5 + (Math.cos(i * 56.78) * 0.5) * this.height * 0.3;
      ...
      if (alpha > 0.05) {
        ...
        this.ctx.beginPath();
        this.ctx.arc(x, y, size, 0, Math.PI * 2);
        this.ctx.fillStyle = `${colors.gradient[colorIndex]}${Math.floor(alpha * 255).toString(16).padStart(2, '0')}`;
        this.ctx.fill();
      }
    }
  }
  ...
  drawGrid(colors, intensity) {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(this.centerX - this.width * 0.5 * perspectiveScale, y);
      this.ctx.lineTo(this.centerX + this.width * 0.5 * perspectiveScale, y);
      this.ctx.stroke();
    }
