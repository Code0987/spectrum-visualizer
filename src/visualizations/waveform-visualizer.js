...
class WaveformVisualizer extends BaseVisualizer {
  ...
  drawWave(data, alpha, offset, colors, position) {
    ...
    const baseY = position === 'bottom' ? this.centerY + offset : this.centerY - offset;
  ...
  drawMainWaveform(data, colors, intensity) {
    ...
    // Fill with gradient
    const fillGradient = this.ctx.createLinearGradient(0, this.centerY - this.height * 0.35, 0, this.centerY + this.height * 0.35);
  ...
  drawFrequencyDecorations(data, colors, intensity) {
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      this.ctx.fillStyle = `${colors.tertiary}${Math.floor(alpha * 255).toString(16).padStart(2, '0')}`;
